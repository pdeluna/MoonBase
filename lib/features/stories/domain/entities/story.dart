...
class Story {
  const Story({
    ...
    this.caption,
    this.archived =
        false, // i know i said status didn't matter (still true) just to be safe let's keep it false since the archive feature is not built yet ;p
    this.syncStatus = SyncStatus
        .synced, //default sync status must exist so phase 4 will be a one-line change - p
  });
  ...
  bool get isExpired => DateTime.now()
      .isAfter(createdAt.add(ttl)); // -> checks if story is expired
  final bool archived;
  ...
  bool operator ==(Object other) {
    ...
    return other is Story &&
        ...
        // safe to include these, we will use these values for unit tests - p
