...
class BaseRepositoryImpl implements BaseRepository {
  ...
  Future<Either<Failure, Base>> createBase(
          {required String name, required UserId ownerUserId}) =>
      guard(() async {
        final m =
            await local.createBase(name: name, ownerUserId: ownerUserId.value);
        return m.toEntity();
      });

  @override
  Future<Either<Failure, Base>> joinBase(
          {required String inviteCode, required UserId userId}) =>
      guard(() async {
        final m =
            await local.joinBase(inviteCode: inviteCode, userId: userId.value);
        return m.toEntity();
      });
  ...
  Future<Either<Failure, void>> leaveBase(
          {required BaseId baseId, required UserId userId}) =>
      guardVoid(
          () => local.leaveBase(baseId: baseId.value, userId: userId.value));

  @override
  Future<Either<Failure, void>> renameBase(
          {required BaseId baseId,
          required String newName,
          required UserId requesterUserId}) =>
      guardVoid(() => local.renameBase(baseId: baseId.value, newName: newName));

  @override
  Future<Either<Failure, void>> deleteBase(
          {required BaseId baseId, required UserId requesterUserId}) =>
  ...
  @override
  Future<Either<Failure, List<Invite>>> listInvitesForBase(
          {required BaseId baseId}) =>
      guardWithTimeout(() async {
  ...
  Future<Either<Failure, Invite?>> getInviteByCode({required String code}) =>
      // Two sequential gets share one [kGuardTimeout] (per-call, not per-get).
      guardWithTimeout(() async {
  ...
  Future<Either<Failure, void>> setLastAccessedBase(
      ...
      guardVoid(() => local.setLastAccessedBase(userId.value, baseId.value));
}
