...
void main() {
  group('BaseRepositoryImpl + InMemory DS', () {
    ...
    test('createBase then listBases includes the created base for owner', () async {
      final created = await repo.createBase(name: 'Home', ownerUserId: 'u1');
      expect(created, isA<Right<Failure, Base>>());
      final base = (created as Right<Failure, Base>).value;

      final listRes = await repo.listBases(userId: 'u1');
    ...
    test('generateInviteCode -> joinBase -> member sees base in list', () async {
      final created = await repo.createBase(name: 'Friends', ownerUserId: 'u9');
      final base = (created as Right<Failure, Base>).value;

      final codeRes = await repo.generateInviteCode(baseId: base.id, requesterUserId: 'u9');
      expect(codeRes, isA<Right<Failure, String>>());
      final code = (codeRes as Right<Failure, String>).value;

      final joinRes = await repo.joinBase(inviteCode: code, userId: 'u2');
      expect(joinRes, isA<Right<Failure, Base>>());

      final listRes = await repo.listBases(userId: 'u2');
      final list = (listRes as Right<Failure, List<Base>>).value;
      expect(list.length, 1);
      expect(list.first.id, base.id);
    });
    ...
    test('renameBase updates name; deleteBase removes it', () async {
      final created = await repo.createBase(name: 'Temp', ownerUserId: 'u1');
      final base = (created as Right<Failure, Base>).value;

      final rn = await repo.renameBase(baseId: base.id, newName: 'Renamed', requesterUserId: 'u1');
      expect(rn, isA<Right<Failure, void>>());

      final afterRename = (await repo.listBases(userId: 'u1') as Right<Failure, List<Base>>).value;
      expect(afterRename.single.name, 'Renamed');

      final del = await repo.deleteBase(baseId: base.id, requesterUserId: 'u1');
      expect(del, isA<Right<Failure, void>>());

      final afterDelete = (await repo.listBases(userId: 'u1') as Right<Failure, List<Base>>).value;
      expect(afterDelete, isEmpty);
    });

    test('joinBase with invalid code returns Left(CacheFailure)', () async {
      final res = await repo.joinBase(inviteCode: 'BAD123', userId: 'u1');
