...
> **Firebase foundation (Phase 4):** `Firebase.initializeApp` now runs at startup in [`lib/main.dart`](../lib/main.dart) using `DefaultFirebaseOptions` from [`lib/firebase_options.dart`](../lib/firebase_options.dart) (generated by `flutterfire configure`; Android config in `android/app/google-services.json`). This wires the SDKs (Core/Auth/Firestore/Storage) only — the data layer (repositories, media storage) is still SharedPreferences/local-file backed. Regenerate the config for your own project with `flutterfire configure`. These generated files hold client config, not server secrets; protect data with Firebase Security Rules and API-key restrictions.
