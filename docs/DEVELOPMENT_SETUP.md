...
| **Flutter SDK** | **3.29.2** (managed via FVM, see Section 3.2) | App framework |
| **Dart SDK** | **3.7.2** (bundled with Flutter 3.29.2) | Language toolchain |
...
> **Prerequisite check:** before continuing, run `git --version`, `fvm flutter --version`, `fvm dart --version`, and `code --version`. Do not use a system-wide `flutter` or `dart` command in this repository.
...
The project pins Flutter **3.29.2** (Dart **3.7.2**) in
`.fvmrc`. FVM guarantees every contributor and CI runner uses the same SDK
version, so "works on my machine" bugs caused by SDK drift cannot happen.
...
| `mime ^2.0.0` | MIME sniffing for media validation (Phase 3; 2.x required by `firebase_storage`) |
...
> **Firebase foundation (Phase 4):** `Firebase.initializeApp` now runs at startup in [`lib/main.dart`](../lib/main.dart) using `DefaultFirebaseOptions` from [`lib/firebase_options.dart`](../lib/firebase_options.dart) (generated by FlutterFire; Android config in `android/app/google-services.json`). The iOS configuration must be generated by an authorized owner and committed as one coherent set before a device run. Follow [`README-ios.md`](README-ios.md#one-time-firebase-owner-step); do not invent app IDs or commit a partial configuration. These generated files hold client config, not server secrets; protect data with Firebase Security Rules and API-key restrictions.
...
- **iOS images-only Firebase pass** — keep photo-specific
  `NSCameraUsageDescription` and `NSPhotoLibraryUsageDescription` in
  `ios/Runner/Info.plist`. Do not declare microphone access while cloud video
  remains deferred.
